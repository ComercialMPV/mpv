...
router.get('/generate/:documentId', auth, async (req, res) => {
  ...
  try {
    ...
    let launchOptions = {
      ...
      timeout: 60000,           // Increased slightly
      args: [
        '--no-sandbox',
    ...
    if (isProduction) {
      ...
      console.log('💻 LOCAL mode - using puppeteer-core');
      
      // Option 1: Try using installed Chrome (Recommended)
      launchOptions.channel = 'chrome';     // Uses system Google Chrome

      // Option 2: If you have Chromium installed via puppeteer, uncomment this:

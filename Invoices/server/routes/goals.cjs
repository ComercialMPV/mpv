...
// GET breakdown by item type - VERSÃO CORRIGIDA (suporta Combo e Subscription)
router.get('/:goalId/breakdown', auth, async (req, res) => {
  try {
    ...
    const breakdown = await Sale.aggregate([
      ...
      // Lookup no Bundle quando o itemType for "bundle"
      {
        $lookup: {
          from: 'bundles',
          localField: 'items.item',        // campo que guarda o _id do Bundle
          foreignField: '_id',
          as: 'bundleInfo'
        }
      },
      { $unwind: { path: '$bundleInfo', preserveNullAndEmptyArrays: true } },

      {
        $group: {
          _id: {
            $cond: {
              if: { $eq: ['$items.itemType', 'bundle'] },           // caso vindo do portal
              then: { $ifNull: ['$bundleInfo.type', 'Bundle'] },    // "Combo" ou "Subscription"
              else: '$items.itemType'                                // Product, Service, etc.
            }
          },
          revenue: { 

// src/templates/public-portal/variants/BoutiquePortal.tsx
import React, { useState, useEffect } from 'react';
import { Service, Company, Product, Bundle} from '../../../services/api';
import {
...
const BoutiquePortal: React.FC<BoutiqueProps> = ({ 
  company, 
  slug, 
  ...
  const handler = setTimeout(() => {
    api.public.logSearch(slug, { term: searchTerm, catalog: activeCatalog }); // ← USE activeCatalog
  }, 500);
  return () => clearTimeout(handler);
}, [searchTerm, slug, activeCatalog]);
  ...
  const loadCatalog = async () => {
    try {
      ...
      // Carregar individualmente o que faltar
      if (!initialServices || initialServices.length === 0) {
        promises.push(api.services.getAll().then(s => setServices(s)));
      } else {
        setServices(initialServices);
      }
      
      if (!initialProducts || initialProducts.length === 0) {
        promises.push(api.products.getAll().then(p => setProducts(p)));
      } else {
        setProducts(initialProducts);
      }
      
      if (!initialBundles || initialBundles.length === 0) {
        promises.push(api.bundles.getAll().then(b => setBundles(b)));
      } else {
  ...
  loadCatalog();
}, [initialServices, initialProducts, initialBundles]);
...
{/* Header Estilo Premium Boutique */}
      ...
      <header className="bg-white py-5">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-8">
         ...
         <div className="flex items-center">
               {company.logo ? (
                 <img src={company.logo} alt={company.name} className="h-5 w-auto" />
               ) : (

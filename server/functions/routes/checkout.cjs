  ...
  try {
    ...
    let payload = {
     ...
     if (method === "mpesa" || method === "emola") {
       let rawPhone = (customerData.phone || '').replace(/\D/g, '');
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    // ── MATCH PendingCheckout por payment_id (Debito devolve payment_id no callback) ──
    const Transaction = require('../models/Transaction.cjs');
    const PendingCheckout = require('../models/PendingCheckout.cjs');
    ...
    // ── IDEMPOTÊNCIA: se Transaction já existe com status final, ignorar ──
    ...
    if (existingTx && (existingTx.status === 'success' || existingTx.status === 'failed')) {
      console.log(`[WEBHOOK] IDEMPOTÊNCIA: Transaction ${external_reference} já ${existingTx.status} — ignorando`);
      return res.status(200).send('OK');
    }
    ...
    if (!pendingCheckout && !existingTx && effectiveMetadata) {
      if (effectiveMetadata.itemsJson) {
        try { effectiveMetadata.items = JSON.parse(effectiveMetadata.itemsJson); } catch (e) { effectiveMetadata.items = []; }
        delete effectiveMetadata.itemsJson;
      }
      if (effectiveMetadata.customerJson) {
        try { effectiveMetadata.customer = JSON.parse(effectiveMetadata.customerJson); } catch (e) { effectiveMetadata.customer = {}; }
      ...
      if (effectiveMetadata?.type === 'subscription') {
        ...
        try {
          ...
          if (plan.price === 0) {
            ...
            await Subscription.findOneAndUpdate({ company: effectiveMetadata.companyId },
            ...
            const sub = await Subscription.findOneAndUpdate({ company: effectiveMetadata.companyId },
            ...
            console.log(`✅ Subscrição PAGA ativada`);
          }
        } catch (subError) { console.error('[Webhook] Erro subscrição:', subError); }
      }
        ...
        if (effectiveMetadata?.isTemplateDocument || effectiveMetadata?.templateId) {
          try {
            ...
            console.log(`✅ Template documento adquirido`);
          } catch (e) { console.error('❌ Erro template documento:', e); }
        }
        if (!effectiveMetadata?.isTemplateDocument && !effectiveMetadata?.templateId) {
          try {
            const Sale = require('../models/Sale.cjs');
            ...
            if (orderItems.length > 0) {
              const newSale = new Sale({
                company: effectiveMetadata.companyId, origin: 'external',
                items: orderItems.map(function(i) { return ({ productId: i.itemId || i.productId || null, name: i.name, quantity: Number(i.quantity || 1), priceAtSale: Number(i.price || i.priceAtSale || 0), itemType: normalizeItemType(i.type || i.itemType || 'Product', i) })}),
                total: saleTotal, amountPaid: saleTotal, remainingBalance: 0,
                paymentMethod: normalizePaymentMethod(effectiveMetadata.paymentMethod) || 'Pendente', status: 'Pago 100%',
                customer: { id: saleCustomer?.id || null, name: saleCustomer?.name || 'Cliente Online', phone: saleCustomer?.phone || '' },
                createdBy: effectiveMetadata.userId || null, partnerId: null
              });
              await newSale.save();
              console.log(`✅ Venda: ${newSale._id}`);
              for (const item of newSale.items) {
                if (item.itemType === 'Product' && item.productId) {
                  await Product.findByIdAndUpdate(item.productId, { $inc: { stockQuantity: -item.quantity } });
                }
              }
            } else { console.warn(`[Webhook] Venda ignorada: items vazios.`); }
        ...
        try {
          await Company.findByIdAndUpdate(effectiveMetadata.companyId, {
            'publicPortal.variant': effectiveMetadata.variantId, 'publicPortal.variantPurchased': true,
            'publicPortal.variantPricePaid': effectiveMetadata.price || 0, 'publicPortal.variantPurchasedAt': new Date(),

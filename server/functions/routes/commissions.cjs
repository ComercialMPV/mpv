...
router.post('/rules', auth, adminOwnerAuth, async (req, res) => {
  try {
    ...
    // Log para debug
    console.log('[POST /rules] Dados recebidos:', { 
      name, 
      frontendTargetType, 
      targetId, 
      userRole,      // ← Adicione este log
      referralProgramRule,
    ...
    if (!userRole || !mongoose.Types.ObjectId.isValid(userRole)) {
        return res.status(400).json({ 
          message: 'Cargo (userRole) é obrigatório e deve ser um ID válido para regras normais',
          receivedUserRole: userRole  // ← Debug info
        });
    }
